...
# Paths
log_dir = "./../logs/tensorboard/" # Path of Teacher Tensorboard
plot_dir = "./plotting_teacher/"
...
for tag, title in tag_title_map.items():
    ...
    # Save figure
    filename = os.path.join(plot_dir, f"{tag.replace('/', '_')}.png")
    plt.savefig(filename, dpi=300) # Generate filename by replacing '/' with '_'
    plt.close()
